...
Der erste Schritt bevor es an die eigentliche implementierung geht ist es eine alternative Bibiliothek zu finden welche die selben Grafiken erstellen kann wie ECharts bzw React-Echarts welche in der alten Webseite genutzt wurden. Dadurch das Echarts so populär ist gab es glücklicherweise auch eine Vue-ECharts variante wodurch die essentiellen Konfigurationen für die dargestellten Grafiken die selben waren war natürlich uns die Arbeit erleichterte. Jetzt muss lediglich der Installations Prozess der Bibiliothek durchgeführt werden lediglich durch die Installation der Bibiliothek und das einbinden in die Vue Komponenten bestand.

\begin{lstlisting}[language={bash}, caption={Installation von Vue-ECharts}]
npm i echarts vue-echarts
\end{lstlisting}

Hiernach kann die Bibiliothek in den Vue Komponenten eingebunden werden und die Grafiken erstellt werden. \cite{vue-echarts}

\subsection{Integration von Echtzeitdaten}

Wie bereits bei der neuen CO2-Rechner implementierung sind die Echtzeitdaten von der Datenbank hierfür relevant und dafür muss wie zuvor sich an dem bisherigen Code orientiert werden und die Datenbankabfragen angepasst werden. Im beispiel für die Fußabdrücke der Nutzer könnte dies wie folgt aussehen:
...
Hierbei wird eine Funktion erstellt welche die Fußabdrücke der Nutzer aus der Datenbank abruft. Die Funktion kann dann in einer \textbf{onMounted} Funktion aufgerufen werden, damit die Daten beim laden der Komponente angezeigt werden.
...
Es ist relativ einfach eine Karte mit Vue-ECharts zu erstellen, aber das wichtigste worüber die Daten in die Grafik gegeben wird und auch gesagt wird wie die Grafik aussehen soll, ob Karte, Balkendiagram oder viele andere Möglichkeiten. Wichtig bevor auf die \textbf{chartOptions} eingegangen wird es ist essentiel das Diagram eine höhe und breite bekommt um korrekt angezeigt zu werden.

Bei den \textbf{chartOptions} handelt es sich um ein Objekt welches die Daten für die Grafik enthält. Hierbei wird die Karte erstellt und die Daten aus der Datenbank abgerufen und entsprechend angepasst, um die Karte zu erstellen. Die Karte könnte wie folgt aussehen:

Dabei wird zunächst die \textbf{onMounted} Funktion erweiter. Sie soll nicht nur die Daten aus der Datenbank laden sondern auch direkt die konkreten Kartendaten erstellen. Danach sieht die Funktion wie folgt aus:
...
\begin{lstlisting}[language={JavaScript}, caption={Beispiels Konfiguration für ECharts Diagramme}]
function getData() {
    return {
        title: {
...
Das Folgende vorgehen konnte dann bei allen Grafiken und Diagrammen durchgeführt werden und somit das Dashboard erfolgreich in die neue Webseite integriert werden. Im folgenden ist ein Screenshot von dem Kartendiagramm in der neuen Webseite zu sehen.
